...
- **Dev server** (`xtask dev-web`) serves static content and exposes a `/ws` WebSocket.
- On asset/file change: server would broadcast `{ type: "asset-changed", url }`; engine invalidates handle and re-fetches via `fetch`.
- Current status: WS listener stubbed; asset reload path stubbed; will wire in next sprint.
